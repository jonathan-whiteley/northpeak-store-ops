...
export type StreamEventHandlers = {
  ...
  /** Genie "show your work": the generated SQL + result grid. */
  onGenieResult?: (result: {
    sql: string | null;
    columns: string[];
    rows: unknown[][];
  }) => void;
...
function dispatch(evt: Record<string, unknown>, h: StreamEventHandlers) {
  ...
  if (type === 'genie.result') {
    h.onGenieResult?.({
      sql: (evt.sql as string | null) ?? null,
      columns: Array.isArray(evt.columns) ? (evt.columns as string[]) : [],
      rows: Array.isArray(evt.rows) ? (evt.rows as unknown[][]) : [],
    });
    return;
  }
