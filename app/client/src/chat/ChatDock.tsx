    ...
    <>
      ...
      {!open && (
        ...
        >
          ...
          />
          <span className="hidden sm:inline">Ask about this store</span>
          <span className="sm:hidden">Ask Genie</span>
        </button>
      )}
      ...
      {open && (
        ...
        <div className="fixed inset-0 sm:inset-auto sm:bottom-0 sm:right-0 z-40 sm:w-[440px] sm:h-[760px] sm:max-h-[92vh] sm:rounded-tl-2xl border-0 sm:border-l sm:border-t border-border bg-card shadow-2xl flex flex-col overflow-hidden">
          ...
          {/* Unity Catalog governs the model calls — marker on the input dock. */}
          <div className="border-t border-border px-3 pt-2 pb-0.5 bg-card">
            <CapabilityMarker icon="unity-catalog" label="Unity AI Gateway" size={13} />
          </div>

          {/* Input */}
          <form
            onSubmit={onSubmit}
            className="px-3 pb-2.5 pt-1 bg-card flex items-end gap-2"
          >
            <input
              value={input}
