...
import { BarChart3, CalendarDays, LayoutDashboard, MessagesSquare, PackageOpen, Plus, Trash2 } from 'lucide-react';
    ...
    <Sidebar collapsible="icon">
      <SidebarHeader>
        <NavLink to="/" onClick={closeOnMobile} className="flex items-center gap-2.5 px-2 py-2 rounded-md hover:bg-sidebar-accent transition-colors">
          <img
            src="/brand/northpeak-mark-only.svg"
            alt=""
            aria-hidden
            width={28}
            height={28}
          ...
          <div className="flex flex-col leading-tight overflow-hidden group-data-[collapsible=icon]:hidden">
            <span className="text-sm font-semibold truncate tracking-tight">
              {config?.branding.appName ?? 'NorthPeak Store Ops'}
            </span>
          </div>
        </NavLink>
      </SidebarHeader>
      ...
      <SidebarContent>
        ...
        {/* Footer — provenance line: the whole loop is governed on Databricks. */}
        <SidebarGroup className="mt-auto group-data-[collapsible=icon]:hidden">
          <SidebarGroupContent className="px-2">
            <div className="rounded-lg border border-sidebar-border/70 px-3 py-2.5">
              <div className="text-[10px] leading-snug text-muted-foreground">
                Data, writes &amp; assistant governed on
              </div>
              <img
                src="/brand/db/databricks-logo-white.svg"
                alt="Databricks"
                className="mt-1.5 w-auto opacity-90"
                style={{ height: 14 }}
              />
            </div>
          </SidebarGroupContent>
