...
export async function fetchTransferLinks(): Promise<StoreTransferLink[]> {
  const res = await okOrThrow(
    await fetch('/api/positions/transfer-links'),
    '/api/positions/transfer-links',
  );
  return res.json();
}
...
/**
 * DIRECT reorder / restock write-back — the manager button (distinct from
 * the agent-mediated Approve/Ask why flow that routes through the chat
 * dock). POSTs to /api/supply/reorder, which INSERTs one `ops_actions` row
 * with move_type='reorder'. Callers should `dataMutated.emit()` on success
 * so the KPIs / queue / map / activity feed refetch.
 */
export async function reorderUnits(args: {
  storeId: string;
  productId: string;
  units: number;
}): Promise<{ action_id: string }> {
  const res = await okOrThrow(
    await fetch('/api/supply/reorder', {
      method: 'POST',
