...
/** A shortfall→nearest-surplus transfer route for the store map's arcs.
 *  One per (shortfall store → surplus store) pair, aggregated across the
 *  SKUs that share the route. Both endpoints carry coords so the map can
 *  draw a weighted arc; `units`/`recapturedUsd` size + label it. */
