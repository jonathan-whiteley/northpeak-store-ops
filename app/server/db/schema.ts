...
export const opsActions = appSchema.table(
  'ops_actions',
  {
    ...
    moveType: text('move_type', {
      enum: ['transfer', 'expedite', 'substitute', 'markdown_hold', 'reorder'],
    }).notNull(),
...
export type ThinkingEntry =
  ...
  /** A Genie "show your work" card: the generated SQL + the query result grid.
   *  Persisted so a page reload re-renders the SQL block + results table. */
