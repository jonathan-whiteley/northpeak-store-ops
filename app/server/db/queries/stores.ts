...
export type StoreTransferLink = {
  ...
  surplusLat: number;
  surplusLng: number;
  /** Summed recommended units across the SKUs on this route (null if unscored). */
  units: number | null;
  /** Summed predicted recaptured $ across the SKUs on this route. */
  recapturedUsd: number | null;
  /** Summed lost-sales exposure the route addresses — the arc's importance. */
  lostSalesExposureUsd: number;
  /** How many shortfall SKUs this route covers. */
  skuCount: number;
};

export async function storeTransferLinks(
  db: AppDb,
): Promise<StoreTransferLink[]> {
  const res = await db.execute(sql`
    WITH store_coords AS (
      SELECT store_id,
    ...
    SELECT
      ...
      sc.lat  AS store_lat,
      sc.lng  AS store_lng,
      s.nearest_surplus_store_id AS surplus_store_id,
      ss.lat  AS surplus_lat,
      ss.lng  AS surplus_lng,
      SUM(rr.recommended_units)                        AS units,
  ...
  return (
    res.rows as Array<{
      store_id: string;
      store_lat: number | string;
      store_lng: number | string;
      surplus_store_id: string;
      surplus_lat: number | string;
      surplus_lng: number | string;
      units: number | string | null;
      recaptured_usd: number | string | null;
      lost_sales_exposure_usd: number | string;
      sku_count: number;
    }>
  ).map((r) => ({
    storeId: r.store_id,
    storeLat: Number(r.store_lat),
    storeLng: Number(r.store_lng),
    surplusStoreId: r.surplus_store_id,
    surplusLat: Number(r.surplus_lat),
    surplusLng: Number(r.surplus_lng),
    units: r.units === null ? null : Number(r.units),
    recapturedUsd: r.recaptured_usd === null ? null : Number(r.recaptured_usd),
...
export async function getRecommendation(
  ...
  const r = res.rows[0] as
    | {
        ...
        move_ranking: unknown;
      }
    | undefined;
  if (!r) return null;

  // Transform moveRanking from snake_case to camelCase
  const transformMoveRanking = (raw: unknown): MoveOption[] => {
    if (!Array.isArray(raw)) return [];
    return raw
      .map((item: any) => {
        // Handle both snake_case (from database) and camelCase (already transformed)
        const move =
          item.move ||
          (item['recommended_move'] === 'transfer' ||
          item['recommended_move'] === 'expedite' ||
          item['recommended_move'] === 'substitute' ||
          item['recommended_move'] === 'markdown_hold'
            ? item['recommended_move']
            : null);
        return {
          move: move as 'transfer' | 'expedite' | 'substitute' | 'markdown_hold',
          units: Number(item.units ?? 0),
          costUsd: Number(item.cost ?? item.costUsd ?? 0),
          predictedRecapturedUsd: Number(
            item.recaptured ?? item.predictedRecapturedUsd ?? 0,
          ),
          predictedNetValueUsd: Number(
            item.net_value ?? item.predictedNetValueUsd ?? 0,
          ),
          sourceStoreId: item.source_store_id ?? item.sourceStoreId ?? null,
          substituteProductId:
            item.substitute_product_id ?? item.substituteProductId ?? null,
        };
      })
      .filter((opt) => opt.move !== null);
  };
  ...
  return {
    ...
    moveRanking: transformMoveRanking(r.move_ranking),
  };
}
...
/**
 * Record an approved recovery action — the Act-layer write. Filter-driven +
 * transactional (TEMPLATE_MAP pattern #5): inputs are the move's fields + the
 * drafted memo, never a list of ids. Inserts one `ops_actions` row for the
 * approved move; for a transfer with a known source, ALSO inserts a paired
 * `markdown_hold` row on the source surplus store (so it isn't discounted
 * while it feeds the transfer). Both writes share one transaction.
 */
export async function recordRecoveryAction(
  ...
  return db.transaction(async (tx) => {
    ...
    const approvedAudit: AuditEntry[] = [
      {
        at: now,
        by: args.userEmail,
        action: 'approved',
    ...
    const primary = await tx.execute(sql`
      INSERT INTO app.ops_actions
        (store_id, product_id, move_type, source_store_id, units,
         drafted_request, predicted_recaptured_usd, status, approved_by,
         audit_trail, decided_at)
      VALUES
        (${args.storeId}, ${args.productId}, ${args.moveType},
         ${args.sourceStoreId}, ${args.units}, ${args.draftedRequest},
         ${args.predictedRecapturedUsd}, 'approved', ${args.userEmail},
         ${JSON.stringify(approvedAudit)}::jsonb, now())
      RETURNING id
    `);
    ...
    if (args.moveType === 'transfer' && args.sourceStoreId) {
      ...
      const hold = await tx.execute(sql`
        INSERT INTO app.ops_actions
          (store_id, product_id, move_type, units, drafted_request,
           status, approved_by, audit_trail, decided_at)
        VALUES
          (${args.sourceStoreId}, ${args.productId}, 'markdown_hold', 0,
           ${holdNote}, 'approved', ${args.userEmail},
           ${JSON.stringify(holdAudit)}::jsonb, now())
        RETURNING id
      `);
      markdownHoldId = (hold.rows[0] as { id: string }).id;
    }
...
/**
 * Record a DIRECT reorder / restock write-back — a manager action distinct
 * from the agent-mediated recovery chain. Unlike `recordRecoveryAction`
 * (which the agent's `execute_recovery_action` tool drives through Genie),
 * this is a single button → REST → Lakebase write: it INSERTs one
 * `ops_actions` row with `move_type='reorder'` and no paired markdown-hold
 * (a reorder expedites fresh stock from the DC; it doesn't drain a surplus
 * store). The Operations queue / KPIs / activity feed cascade off this row
 * exactly like a recovery move because `listPositions` / `recentActivity`
 * already read every non-markdown ops_actions row.
 */
export async function recordReorder(
  db: AppDb,
  args: {
    storeId: string;
    productId: string;
  ...
  return db.transaction(async (tx) => {
    const now = new Date().toISOString();
    const label = args.productName ? ` of ${args.productName}` : '';
    const draftedRequest = `Reorder ${args.units} units${label} — expedite from DC to refill ${args.storeId}`;
    const auditTrail: AuditEntry[] = [
      {
        at: now,
        by: args.userEmail,
        action: 'approved',
        notes: 'Reorder released',
        tool: 'reorder',
      },
    ];
    const inserted = await tx.execute(sql`
      INSERT INTO app.ops_actions
        (store_id, product_id, move_type, source_store_id, units,
         drafted_request, status, approved_by, audit_trail, decided_at)
      VALUES
        (${args.storeId}, ${args.productId}, 'reorder', NULL, ${args.units},
         ${draftedRequest}, 'approved', ${args.userEmail},
         ${JSON.stringify(auditTrail)}::jsonb, now())
      RETURNING id
    `);
