...
export type ToolProgressEvent =
  ...
  /** Genie "shows its work" — the generated SQL + the query's result grid.
   * Emitted by `callGenieSpace` once the poll COMPLETES and the query result
   * has been fetched. The UI renders a Generated-SQL block + a grain-adaptive
   * results table alongside the streamed answer. `sql` may be null (text-only
   * Genie answer); `columns`/`rows` may be empty (no result grid available). */
