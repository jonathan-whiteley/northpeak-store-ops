...
/**
 * Fetch a Genie query attachment's result grid (columns + rows). Genie
 * returns the query TEXT inline in the message poll, but the actual result
 * rows live behind a separate REST call. We try the attachment-scoped
 * endpoint first (when we have an attachment id), then fall back to the
 * message-level query-result endpoint, then — if the response carries only a
 * statement_id (row data lives in the SQL Statement Execution API) — fetch
 * the statement. Mirrors the reference app's genie.py, adapted to raw REST.
 * Never throws; returns empty columns/rows on any failure so the tool still
 * returns its synthesized answer.
 */
async function fetchGenieQueryResult(
  ctx: DataToolContext,
  spaceId: string,
  ...
  async function getJson(url: string): Promise<StatementResponse | null> {
    try {
      ...
      if (!resp.ok) {
        console.warn('[ask_genie] query-result fetch non-2xx', {
          url,
          status: resp.status,
        });
        return null;
      }
      return (await resp.json()) as StatementResponse;
    } catch (e) {
      console.warn('[ask_genie] query-result fetch failed', (e as Error).message);
      return null;
    }
  }

  const base = `${ctx.databricksHost}/api/2.0/genie/spaces/${spaceId}/conversations/${conversationId}/messages/${messageId}`;
  let payload: StatementResponse | null = null;
  if (attachmentId) {
    payload = await getJson(`${base}/attachments/${attachmentId}/query-result`);
  }
  if (!payload?.statement_response) {
    payload = await getJson(`${base}/query-result`);
  }

  const stmt = payload?.statement_response;
  if (!stmt) return { columns: [], rows: [] };

  let columns =
    stmt.manifest?.schema?.columns?.map((c, i) => c.name ?? String(i)) ?? [];
  let rows: unknown[][] = Array.isArray(stmt.result?.data_array)
    ? (stmt.result!.data_array as unknown[][])
    : [];

  // Row data can live behind the SQL Statement Execution API when the Genie
  // query-result response carries only the manifest + a statement_id.
  if (rows.length === 0 && stmt.statement_id) {
    const raw = await getJson(
      `${ctx.databricksHost}/api/2.0/sql/statements/${stmt.statement_id}`,
    );
    const inner = raw as unknown as {
      manifest?: { schema?: { columns?: Array<{ name?: string }> } };
      result?: { data_array?: unknown[][] };
    } | null;
    if (inner) {
      if (columns.length === 0) {
        columns =
          inner.manifest?.schema?.columns?.map((c, i) => c.name ?? String(i)) ??
          [];
      }
      if (Array.isArray(inner.result?.data_array)) {
        rows = inner.result!.data_array as unknown[][];
      }
    }
  }

  // Cap rows so a huge grid can't bloat the SSE payload / persisted thinking.
...
export async function callGenieSpace(
  ...
  for (let attempts = 0; attempts < POLL_MAX_ATTEMPTS; attempts++) {
    ...
    if (pollData.status === 'COMPLETED') {
      ...
      const parts: string[] = [];
      // Capture the FIRST query attachment's generated SQL + id so we can
      // fetch its result grid and surface both to the UI ("show your work").
      let sql: string | null = null;
      let queryAttachmentId: string | null = null;
      for (const att of pollData.attachments ?? []) {
        if (att.text?.content) parts.push(att.text.content);
        if (att.query?.description) parts.push(att.query.description);
        if (sql === null && att.query?.query) {
          sql = att.query.query;
          queryAttachmentId = att.attachment_id ?? null;
        }
      }
      answer = parts.join('\n\n') || '(Genie returned no text content)';

      // "Show your work": fetch the result grid for the generated SQL and
      // emit it (plus the SQL) to the Thinking panel. Best-effort — a
      // text-only answer (no SQL) or a failed fetch just skips the card.
      if (sql) {
        const { columns, rows } = await fetchGenieQueryResult(
          ctx,
          spaceId,
          conversation_id,
          message_id,
        ...
        emit({ kind: 'genie_result', sql, columns, rows });
      }
      break;
