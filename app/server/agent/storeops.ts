    ...
    fetch: async (input, init) => {
      ...
      if (typeof body === 'string' && body.startsWith('{')) {
        try {
          ...
          // Databricks' Anthropic (Claude) chat-completions passthrough rejects
          // top-level fields the Agents SDK derives from modelSettings:
          // `reasoning_effort` (from modelSettings.reasoning) and `store` (from
          // modelSettings.store) both come back "Extra inputs are not permitted".
          // These are Responses-API concepts; the Responses path carries them
          // differently (a `reasoning` object / server-side state), so stripping
          // these scalars is safe for both APIs. Same class of fix as `strict`.
          for (const k of ['reasoning_effort', 'store'] as const) {
          ...
          // Databricks' Anthropic (Claude) passthrough rejects the `strict`
          // field the Agents SDK stamps on tool schemas over chat-completions
          // ("tools.0.custom.strict: Extra inputs are not permitted"). Strip it
          // wherever it appears so Claude tool-calling works.
          if (Array.isArray(parsed.tools)) {
            for (const t of parsed.tools) {
              if (t && typeof t === 'object') {
                delete (t as Record<string, unknown>).strict;
                const fn = (t as { function?: Record<string, unknown> }).function;
                if (fn && typeof fn === 'object') delete fn.strict;
                const cu = (t as { custom?: Record<string, unknown> }).custom;
                if (cu && typeof cu === 'object') delete cu.strict;
              }
            }
          }
