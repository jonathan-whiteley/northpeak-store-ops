  ...
  try {
    const ctx: AgentContext = {
      ...
      onToolProgress: (ev) => {
        ...
        } else if (ev.kind === 'genie_result') {
          // Genie "show your work": the generated SQL + result grid. Persist
          // it in the reasoning trail AND stream it live so the client can
          // render the SQL block + results table (see ThinkingPanel).
          thinking.push({
            kind: 'genie_result',
            sql: ev.sql,
            columns: ev.columns,
