...
export function registerStoreRoutes(app: Application, deps: Deps): void {
  ...
  // --- GET /api/positions/transfer-links (map transfer arcs) -------------
  // Shortfall→nearest-surplus routes with both endpoints' coords + the
  // recommended units / predicted recaptured $. Drives the StoreMap arcs.
  // MUST be registered before '/:id' so 'transfer-links' isn't captured as
  // an :id param.
  app.get('/api/positions/transfer-links', async (_req, res) => {
    res.json(await storeTransferLinks(db));
  });
